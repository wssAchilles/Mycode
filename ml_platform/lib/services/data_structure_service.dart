...
class DataStructureService {
  ...
  ListNode<T>? _cloneList<T>(ListNode<T>? head) {
    ...
    var newCurr = newHead;
    while (curr != null) {
      newCurr.next = ListNode<T>(curr.value);
  ...
  List<DataStructureStep> linkedListInsert(ListNode<int>? head, int value, int index) {
    ...
    if (index == 0) {
      ...
      newHead.next = _cloneList(head); // 注意这里逻辑，实际上我们是在构建新状态
      ...
      steps.add(DataStructureStep(
        ...
        highlightedElements: ['new', '0'], // 约定的ID
      ));
      return steps;
    }

    // 模拟遍历
    var currentHead = _cloneList(head); // 用于模拟过程的临时链表
    ...
    while (curr != null && currentIndex < index - 1) {
      steps.add(DataStructureStep(
        ...
        currentState: currentHead, // 链表结构未变，高亮变了
    ...
    if (curr != null) {
      ...
      steps.add(DataStructureStep(
        ...
        currentState: currentHead, // 此时 currentHead 已经包含了新节点
