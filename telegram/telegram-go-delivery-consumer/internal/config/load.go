...
func Load() Config {
	...
	return Config{
		...
		PlatformReplayWorkerEnabled: readBool(
		...
		SyncWakeExecutionMode:   readPlatformExecutionMode("DELIVERY_CONSUMER_SYNC_WAKE_EXECUTION_MODE", "shadow"),
		...
		PendingIdleDuration: time.Duration(readInt("DELIVERY_CONSUMER_PENDING_IDLE_MS", defaultPendingIdleMS, 1000, 3600000)) * time.Millisecond,
		PendingClaimCount: int64(readInt(
			"DELIVERY_CONSUMER_PENDING_CLAIM_COUNT",
			readInt("DELIVERY_CONSUMER_READ_COUNT", defaultReadCount, 1, 500),
			1,
			500,
		)),
		PendingClaimInterval: time.Duration(readInt("DELIVERY_CONSUMER_PENDING_CLAIM_INTERVAL_MS", defaultPendingClaimIntervalMS, 1000, 3600000)) * time.Millisecond,
		PendingReclaimMaxBatches: readInt(
			"DELIVERY_CONSUMER_PENDING_RECLAIM_MAX_BATCHES",
			defaultPendingReclaimMaxBatches,
			1,
			100,
		),
