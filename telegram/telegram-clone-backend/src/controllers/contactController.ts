 ...
 * 删除联系人
 */
export const removeContact = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
 ...
 * 屏蔽联系人
 */
export const blockContact = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const { contactId } = req.params;
  const userId = req.user?.id;
 ...
 * 更新联系人备注
 */
export const updateContactAlias = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const { contactId } = req.params;
