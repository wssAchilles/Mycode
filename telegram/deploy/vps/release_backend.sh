...
ssh "${REMOTE_TARGET}" \
  "REMOTE_ROOT='${REMOTE_ROOT}' RELEASE_ID='${RELEASE_ID}' ARCHIVE_NAME='${ARCHIVE_NAME}' BACKEND_IMAGE='${BACKEND_IMAGE}' GATEWAY_IMAGE='${GATEWAY_IMAGE}' DELIVERY_CONSUMER_IMAGE='${DELIVERY_CONSUMER_IMAGE}' RECOMMENDATION_IMAGE='${RECOMMENDATION_IMAGE}' GRAPH_KERNEL_IMAGE='${GRAPH_KERNEL_IMAGE}' GHCR_USERNAME='${GHCR_USERNAME}' GHCR_TOKEN='${GHCR_TOKEN}' bash -s" <<'EOF'
set -euo pipefail
...
if [[ -z "${RELEASE_ID}" || -z "${ARCHIVE_NAME}" || -z "${BACKEND_IMAGE}" || -z "${GATEWAY_IMAGE}" || -z "${DELIVERY_CONSUMER_IMAGE}" || -z "${RECOMMENDATION_IMAGE}" || -z "${GRAPH_KERNEL_IMAGE}" ]]; then
  echo "REMOTE_ROOT/RELEASE_ID/ARCHIVE_NAME/BACKEND_IMAGE/GATEWAY_IMAGE/DELIVERY_CONSUMER_IMAGE/RECOMMENDATION_IMAGE/GRAPH_KERNEL_IMAGE must be exported by caller" >&2
  exit 1
fi
