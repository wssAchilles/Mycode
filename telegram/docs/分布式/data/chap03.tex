...
系统总体架构如图\ref{fig:system_architecture_springboot}所示。整体分为客户端层、后端服务层、数据与缓存层、推荐算法层四部分。客户端层包括React前端和Apifox接口联调工具；后端服务层以Spring Boot为核心，负责REST接口、认证与权限、推荐Feed服务、用户与群组服务、新闻内容服务；数据与缓存层包括Redis、Supabase PostgreSQL和对象存储；推荐算法层负责多路召回、过滤规则、加权评分、TopK排序和埋点实验。
...
\begin{longtable}{p{0.13\textwidth}p{0.33\textwidth}p{0.42\textwidth}}
\caption{核心接口设计}\\
\toprule
方法 & 路径 & 说明 \\
\midrule
