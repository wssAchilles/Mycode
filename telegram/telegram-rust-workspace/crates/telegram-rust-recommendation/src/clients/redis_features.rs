...
/// Score: timestamp (ms), Member: `{type}:{postId}:{timestamp}`
...
pub struct RealtimeUserFeatures {
    ...
    pub recency_score: f64, // 0.0-1.0, decay-weighted
}
...
/// Reads real-time user features from Redis
...
impl RealtimeFeatureProvider {
    ...
    /// Fetch real-time features for a user
    pub async fn get_user_features(&self, user_id: &str) -> Result<RealtimeUserFeatures> {
        ...
        let one_hour_ago = now_ms.saturating_sub(3600 * 1000);
        let one_day_ago = now_ms.saturating_sub(86400 * 1000);

        // Get all members in last 24h
        ...
        for member in &members {
            ...
            *type_dist.entry(action_type.to_string()).or_insert(0) += 1;
            ...
            let age_hours = (now_ms.saturating_sub(timestamp)) as f64 / 3600_000.0;
