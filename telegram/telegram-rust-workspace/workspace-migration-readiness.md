...
状态：Phase 16C replay schema 收口完成；服务本体尚未纳入真实 Cargo workspace。
...
- `workspaceMigrationState`：`replay_schema_extracted`
...
5. `telegram-source-primitives`
   - 来源：`telegram-rust-recommendation/src/sources/mod.rs`。
...
- `/ops/recommendation/summary` 输出 `workspaceMigrationState=replay_schema_extracted`。
- `telegram-rust-recommendation` replay 全部通过。
- `telegram-rust-gateway` 可以独立通过 `cargo test`。
- contract、fixtures、ranking primitives、selector primitives 与 source primitives crate 继续由 recommendation service 通过 path dependency 消费。
- `telegram-recommendation-contracts` 继续通过 path dependency 消费 `telegram-rust-http-types` 并 re-export `SuccessEnvelope`。
- 下一批工作只允许做服务纳入 workspace 前的收口审计，而不是直接搬服务目录。
...
Phase 16C 当前完成 replay schema 收口。下一步仍不应该直接移动服务目录；应做服务纳入 workspace 前的收口审计，并保持 `telegram-rust-recommendation` 与 `telegram-rust-gateway` 的服务本体独立构建。
