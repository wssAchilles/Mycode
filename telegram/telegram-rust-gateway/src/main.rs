...
use traffic_policy::TrafficClass;
...
fn spawn_dependency_probe_loop(state: AppState) {
...
async fn proxy_handler(
    ...
    let decision = if route_class.bypass_rate_limit() {
        ...
        if !decision.allowed {
            warn!(client_ip = %client_ip, route_class = %route_class, retry_after_secs = decision.retry_after_secs, "gateway rate limit hit");
...
async fn probe_socket_io_compat(state: &AppState) -> Result<()> {
