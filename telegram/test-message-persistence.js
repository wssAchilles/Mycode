...
try {
  ...
  // 回退到后端项目的 node_modules
  const backendMongoosePath = path.resolve(__dirname, 'telegram-clone-backend/node_modules/mongoose');
...
try {
  ...
  const backendDotenvPath = path.resolve(__dirname, 'telegram-clone-backend/node_modules/dotenv');
  dotenv = require(backendDotenvPath);
}

// 优先加载后端目录下的 .env（包含 MONGODB_URI）
try {
...
async function testMessagePersistence() {
  ...
  try {
    ...
    if (!mongoUri) {
      throw new Error('环境变量 MONGODB_URI 未设置，请在 .env 或 telegram-clone-backend/.env 中配置 Atlas 连接字符串');
    }
    // 安全打印连接字符串（隐藏账号密码）
    try {
